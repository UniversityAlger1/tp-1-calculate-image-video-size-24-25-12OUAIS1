...
int main() {
    ...
    printf("\nEnter width, height, and output u (bt, ko, mo, go) for the colored image: ");
    scanf("%d %d %s", &width, &height, u);
    float coloredSize = coloredBitmap(width, height, u);
    printf("Colored image size: %.2f %s\n", coloredSize, u);


    printf("\nEnter width, height, duration of the movie (seconds), duration of the credits (seconds), FPS, and output u (bt, ko, mo, go): ");
